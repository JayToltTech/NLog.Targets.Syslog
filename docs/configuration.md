...
## Message send settings
 ...
 - `tcp` - settings related to TCP:
    ...
    - `tls` - settings related to TLS:
       ...
       - `pinnedCaCertificatePath` - path to a CA certificate file the server certificate chain must terminate at; when set, the server certificate is validated by pinning to this CA independently of the OS trust store; when omitted default platform validation is used (default: none). The file is loaded via `X509Certificate2`: DER-encoded certificates (`.cer`/`.crt`) work on all targets, while PEM is accepted on .NET Framework/Windows but is not guaranteed on every netstandard2.0 runtime — prefer DER for portability.
